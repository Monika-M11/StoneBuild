...
// ── Tab Components ────────────────────────────────────────────
import AttendanceTab from './Projectabs/Attendance';
// Uncomment as you build each tab:
// import PartyTab       from './ProjectTabs/Party';
// import TransactionTab from './ProjectTabs/Transaction';
// import SiteTab        from './ProjectTabs/Site';
// import TaskTab        from './ProjectTabs/Task';

// ── Placeholder for tabs not yet built ───────────────────────
function PlaceholderTab({ icon, title, sub }: { icon: string; title: string; sub: string }) {
  return (
    <View style={tabStyles.center}>
      <Ionicons name={icon as any} size={48} color={Colors.light.primaryDark + '40'} />
      <DefaultText style={tabStyles.title}>{title}</DefaultText>
      <DefaultText style={tabStyles.sub}>{sub}</DefaultText>
    </View>
  );
}
...
export default function ProjectDetailScreen() {
  ...
  const renderBody = () => {
    switch (activeTab) {
      case 'attendance':
        return <AttendanceTab projectId={id} />;
      case 'party':
        return <PlaceholderTab icon="people-outline" title="Party" sub="Party list will appear here" />;
      case 'transaction':
        return <PlaceholderTab icon="swap-horizontal-outline" title="Transaction" sub="Transaction records will appear here" />;
      case 'site':
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.container}>
              ...
              {TABS.map((tab) => (
                ...
                >
                  <DefaultText
                    style={[
                      styles.tabLabel,
                      { fontFamily: activeTab === tab.key ? theme.fonts.bold : theme.fonts.regular },
                      activeTab === tab.key && styles.tabLabelActive,
                    ]}
                  >
                    {tab.label}
          ...
          {/* ── Body — only this swaps on tab change ── */}
          <View style={styles.body}>
            {renderBody()}
          </View>
